# LidAR + Empty Slots + Dynamic Cars + Rerouting 
# (rerouting is making the code very heavy and crashing the output)

# Yet to implement smoothning in this + external factors
...
def find_all_paths_and_choose_closest_with_lidar():
    ...
    detected_obstacles.extend([dynamic_car["rect"] for dynamic_car in dynamic_cars])

    # Add yellow cars that are dynamic in nature

    # for dyn_car in dynamic_cars:
        # detected_obstacles.append(dyn_car["rect"])
...
def real_time_rerouting():
    """
    Continuously checks for obstacles in the car's path and recalculates the route if necessary.
    """
    global waypoints, target_slot, all_paths

    # Check for obstacles in the path using LiDAR sensor
    detected_objects = lidar_sensor(car, parking_spots, obstacles, num_rays=72, max_range=300)
    unique_detected_objects = log_detected_obstacles(detected_objects)

    # If an obstacle is detected that wasn't there earlier, recalculate path
    if detected_objects:  # If there are new detected obstacles
        print(f"Detected obstacles: {detected_objects}")
        
        # Find a new target slot and path based on new obstacles
        target_slot, waypoints = find_all_paths_and_choose_closest_with_lidar()
        print(f"New target slot selected: {target_slot}, Waypoints: {waypoints}")
...
# Main loop with real-time rerouting logic
...
#     # Draw parking spots and other visuals (same as original)
...
#     # Debugging statements for waypoints and car movement
#     if target_slot:
#         print(f"Target slot: {target_slot}")
#         print(f"Waypoints: {waypoints}")

#     if waypoints:
#         print(f"Car position: {car.center}")
#         car_parked = move_car_along_path(waypoints)
#         print(f"Car parked: {car_parked}")

    

#     # Real-time rerouting check
#     if not car_parked:
#         real_time_rerouting()  # Continuously check for rerouting based on LiDAR sensor

#     # Draw paths and car movement
#     if target_slot:
#         pygame.draw.rect(screen, YELLOW, target_slot, 4)  # Highlight the target slot
#         for i in range(len(waypoints) - 1):
#             pygame.draw.line(screen, (0, 0, 0), waypoints[i], waypoints[i + 1], 4)  # Green path
...
# Helper function to calculate distance between two points (previous and current position)
def distance_between(pos1, pos2):
    return ((pos1[0] - pos2[0])**2 + (pos1[1] - pos2[1])**2) ** 0.5

# Track car's position for progress checking
previous_position = car.center
progress_threshold = 2  # Threshold to check if the car has moved (e.g., 2 pixels)
no_progress_counter = 0  # Counter to track if no progress has been made
# Track the number of frames since the last rerouting check
rerouting_frame_interval = 20  # Check rerouting every 10 frames
rerouting_frame_counter = 0
# Main loop with progress-based rerouting check
simulation_running = True
unique_detected_objects = []
while running:
    screen.fill(LIGHT_BLUE)
    ...
    # Real-time rerouting only if no progress is made
    if not car_parked:
        rerouting_frame_counter += 1
        current_position = car.center
        if rerouting_frame_counter >= rerouting_frame_interval:
            rerouting_frame_counter = 0  # Reset the counter
            real_time_rerouting()  # Trigger rerouting based on LiDAR sensor
        print("Rerouting triggered")
        if distance_between(previous_position, current_position) < progress_threshold:
            no_progress_counter += 1
            if no_progress_counter > 3:  # If car has made no progress for 3 frames, reroute
                real_time_rerouting()  # Trigger rerouting based on LiDAR sensor
                print("Car is not making progress, rerouting...")
                no_progress_counter = 0  # Reset the counter after rerouting
        else:
            no_progress_counter = 0  # Reset counter if car is making progress

        # Update previous position for the next check
