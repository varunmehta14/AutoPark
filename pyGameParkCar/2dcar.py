...
# Create parking spots and mark them as occupied or empty
for row in range(NUM_ROWS):
    ...
    for col in range(NUM_SPOTS_PER_ROW):
        ...
        occupied = (row, col) in occupied_spots  # Mark the spot as occupied if it's in the set
...
# Random car placement function
def place_random_cars():
    for _ in range(NUM_OCCUPIED_SPOTS):
        # Select a random spot
        spot = random.choice(parking_spots)
        # Mark it as occupied
        spot["occupied"] = True

# LIDAR-like function to detect if a parking spot is occupied (within a certain radius)
def lidar_check(parking_spot, radius=30):
    car_center = car.center
    spot_center = parking_spot["rect"].center
    distance = math.hypot(spot_center[0] - car_center[0], spot_center[1] - car_center[1])
    return distance < radius  # If within LIDAR radius, treat it as occupied
...
# Place random cars in parking spots
place_random_cars()
...
while running:
    ...
    for slot, path, _ in all_paths:
        for i in range(len(path) - 1):
            pygame.draw.line(screen, (211, 211, 211), path[i], path[i + 1], 2)

    # Highlight the closest path in green
    if waypoints:
        for i in range(len(waypoints) - 1):
            pygame.draw.line(screen, GREEN, waypoints[i], waypoints[i + 1], 3)

    # Draw the target parking slot
    if target_slot:
        pygame.draw.rect(screen, YELLOW, target_slot)
    ...
    # Draw car
    screen.blit(car_image, car.topleft)
